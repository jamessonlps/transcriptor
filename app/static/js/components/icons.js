// Single source of truth for SVG icons used across the app.
//
// All paths are 24x24 viewBox, currentColor stroke. Consumers control
// size and colour via CSS on the wrapping element. The HTML returned
// here is intended for ``innerHTML`` injection — never construct strings
// from untrusted input that goes through this.

const SVG = (paths, opts = {}) => {
    const stroke = opts.fillBased ? '' : ' fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"';
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"${stroke}>${paths}</svg>`;
};
...
export const ICONS = {
    ...
    trash: SVG('<polyline points="3 6 5 6 21 6"/><path d="M19 6l-2 14a2 2 0 0 1-2 2H9a2 2 0 0 1-2-2L5 6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/>'),
...
/**
 * Return the SVG markup for an icon by name. Use ``innerHTML`` to inject.
 * Unknown names return an empty string — callers can treat that as "no icon".
 *
...
/**
 * Convenience: create a detached DOM element pre-populated with an icon.
 * Useful when you don't want to use ``innerHTML`` for some reason (CSP, etc.).
 */
export function iconElement(name, { tag = 'span', className = '' } = {}) {
...
/**
 * Walk a DOM subtree and replace any ``<i data-icon="X"></i>`` (or
 * ``<span data-icon="X">``) placeholder with the matching SVG. Lets the
 * HTML templates stay free of inline SVG markup.
 *
